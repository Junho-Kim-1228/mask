...
#         outer_mask = cv2.dilate(outer_mask, k3, iterations=12)
...
#     cv2.imwrite('output/mask_type_C.png', white_shape_rgba)
#     print("✅ 완료: output/mask_type_C.png")
...
def create_white_shape_bmp():
    ...
    # 마스크 재구성 (확장/축소 로직)
    # 두께 조절: iterations 숫자를 변경 (예: 12)
    outer_mask = np.zeros_like(mask_main)
    if outer_s is not None:
        cv2.drawContours(outer_mask, [outer_s], -1, 255, thickness=cv2.FILLED)
        outer_mask = cv2.dilate(outer_mask, k3, iterations=30)
    ...
    # ✅ [변경됨] 배경 검정(0), 코일 흰색(255)인 3채널 BMP 이미지 생성
    # final_mask는 흑백(1채널)이므로 컬러(3채널) 포맷으로 변환해야 일반적인 BMP로 저장됨
    result_bmp = cv2.cvtColor(final_mask, cv2.COLOR_GRAY2BGR)

    # 저장
    os.makedirs('output', exist_ok=True)
    cv2.imwrite('output/mask_type_C.bmp', result_bmp)
    print("✅ 완료: output/mask_type_C.bmp")

if __name__ == "__main__":
    create_white_shape_bmp()